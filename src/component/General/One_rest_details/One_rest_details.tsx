...
const OneRestDetails: React.FC = () => {
  ...
  const allDishesState = useSelector((state: IRootState) => state.dishes.value);

  const restobj = allRestaurants.find(
    (rest: IrestaurantsValue) => detailsrest.id == rest.id
  );
  console.log(restobj)
  const newArrDishes = restobj?.dishes?.map((dishId: any) => {
    const singleDish = allDishesState.filter(
    (dishObj: IdishesValue) => dishObj.id == dishId
    )[0];
    if(singleDish !== undefined) return singleDish;
    return {};
  });
    ...
    <div className={"one-rest-details-all"}>
      ...
      <div className="all-buttons-one-rest">
        <button
          value={"Breakfast"}
          name="breakfast"
          onClick={(e: React.MouseEvent<unknown>) => update(e)}
